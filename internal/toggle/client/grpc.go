...
func NewGRPCClient(l logger.Logger, jwt string, baseURL string) (*GRPCClient, error) {
	...
	kacp := keepalive.ClientParameters{
		Time:                40 * time.Second, // send pings every 180 seconds if there is no activity
		Timeout:             20 * time.Second, // wait 60 seconds for ping ack before considering the connection dead
		PermitWithoutStream: true,             // only send pings when there are active streams (server may disable pings without streams)
	}
