...
// integration_legacy_test.go - Legacy Event System Integration Tests
//
// This file contains integration tests for the original (legacy) event system that uses:
// - String-based event types (e.g., "system.test.info", "storage.test.warn")
// - map[string]interface{} payloads (unstructured JSON)
// - Backward compatibility functions (EmitSystemEvent, EmitStorageEvent, etc.)
// - Generic payload marshaling in emitEvent()
//
// These tests validate the original event emission patterns during the migration period
// and ensure backward compatibility is maintained. For the new structured event system
// tests, see integration_test.go.
//
// Key characteristics of legacy events:
// - Event types are free-form strings
// - Payloads can be any JSON-serializable data
// - No schema validation or type safety
// - Uses the original 5 event categories
...
func TestEventsGRPC_Integration(t *testing.T) {
	...
	t.Run("DiskFlushFunctionality", func(t *testing.T) {
		testDiskFlushFunctionality(t, l)
	})
}

// testDirectEventClient tests the event client directly
func testDirectEventClient(
	t *testing.T,
...
func testEventSystemInitialization(
	t *testing.T,
	mockServer *MockToggleServer,
	gClient *client.GRPCClient,
	ctx context.Context,
	...
	EmitStorageEvent(
		...
		LevelWarn,
		"test-storage",
		map[string]any{"size": 1024},
		map[string]string{"pool": "test"},
	)
	EmitNetworkEvent(
		"network.test.error",
		LevelError,
...
func testDiskFlushFunctionality(t *testing.T, l logger.Logger) {
	...
	t.Run("BulkFlushAtThreshold", func(t *testing.T) {
		...
		assert.Equal(
			...
			1,
			buffer.Size(),
			"Buffer should have 1 event after flush (correct behavior)",
		)
	...
	t.Run("MultipleDifferentFlushFiles", func(t *testing.T) {
		...
		assert.NotEqual(
			...
			filenames[0],
			filenames[1],
			"Flush files should have different UUID7 names",
		)
		assert.True(
			t,
			filenames[0] != firstFilename || filenames[1] != firstFilename,
			"Second file should be different from first",
		)
		...
		for _, filename := range filenames {
			...
			assert.Regexp(
				...
				`^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$`,
				uuidPart,
				"All files should be UUID7 format",
			)
		}
